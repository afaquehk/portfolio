...
export function EducationCard({ education, index, isLast }) {
  return (
    <div className="relative pl-8 md:pl-0">
      <div className="hidden md:flex items-center justify-between w-full relative">
        ...
        >
          <div className={`flex flex-col gap-1 mb-4 ${index % 2 === 0 ? 'items-end' : 'items-start'}`}>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">{education.title}</h3>
            <span className="text-indigo-600 dark:text-cyan-400 font-medium">{education.institution}</span>
            <div className={`flex items-center gap-2 text-slate-500 dark:text-gray-400 text-sm mt-1 ${index % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
          ...
          <ul className={`space-y-2 mb-4 text-slate-600 dark:text-gray-300 text-sm ${index % 2 === 0 ? 'text-right' : 'text-left'}`}>
            {education.details.map((item, detailIndex) => (
      ...
      <div className="md:hidden relative border-l-2 border-black/10 dark:border-white/10 pb-8 pl-6">
        ...
        >
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">{education.title}</h3>
          <span className="text-indigo-600 dark:text-cyan-400 font-medium">{education.institution}</span>
          <div className="flex flex-wrap items-center gap-2 text-slate-500 dark:text-gray-400 text-xs mt-2 mb-4">
            <span className="flex items-center gap-1"><GraduationCap size={12} /> {education.period}</span>
            <span>|</span>
            <span>{education.location}</span>
          </div>
